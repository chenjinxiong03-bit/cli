...
var (
	driveDeletePollAttempts = 30
	driveDeletePollInterval = 2 * time.Second
)

// Folder delete tasks use the same backend task_check endpoint as folder move,
// but keep a delete-specific wrapper so polling messages and terminal errors
// can stay explicit about the operation being performed.
type driveDeleteTaskCheckStatus struct {
	TaskID string
	Status string
}

func (s driveDeleteTaskCheckStatus) Ready() bool {
	return strings.EqualFold(strings.TrimSpace(s.Status), "success")
}

func (s driveDeleteTaskCheckStatus) Failed() bool {
	status := strings.TrimSpace(s.Status)
	return strings.EqualFold(status, "fail") || strings.EqualFold(status, "failed")
}

func (s driveDeleteTaskCheckStatus) StatusLabel() string {
	status := strings.TrimSpace(s.Status)
	if status == "" {
		return "unknown"
	}
...
var DriveDelete = common.Shortcut{
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if spec.FileType == "folder" {
			...
			status, ready, err := pollDriveDeleteTaskCheck(runtime, taskID)
			if err != nil {
				return err
			}
...
func getDriveDeleteTaskCheckStatus(runtime *common.RuntimeContext, taskID string) (driveDeleteTaskCheckStatus, error) {
	parsed, err := getDriveTaskCheckStatus(runtime, taskID)
	if err != nil {
		return driveDeleteTaskCheckStatus{}, err
	}

	return driveDeleteTaskCheckStatus{
		TaskID: parsed.TaskID,
		Status: parsed.Status,
	}, nil
}

// pollDriveDeleteTaskCheck bounds the wait time for folder deletes so callers
// can fall back to drive +task_result when the backend keeps running.
func pollDriveDeleteTaskCheck(runtime *common.RuntimeContext, taskID string) (driveDeleteTaskCheckStatus, bool, error) {
	lastStatus := driveDeleteTaskCheckStatus{TaskID: taskID}
	for attempt := 1; attempt <= driveDeletePollAttempts; attempt++ {
		if attempt > 1 {
			time.Sleep(driveDeletePollInterval)
		}

		status, err := getDriveDeleteTaskCheckStatus(runtime, taskID)
		if err != nil {
		...
		lastStatus = status
		if status.Ready() {
			fmt.Fprintf(runtime.IO().ErrOut, "Folder delete completed successfully.\n")
			return status, true, nil
		}
		if status.Failed() {
			return status, false, output.Errorf(output.ExitAPI, "api_error", "folder delete task failed")
		}
	}
