# lark-slides_ai xml_presentation.sildes delete
...
```bash
lark-cli slides_ai xml_presentation.sildes delete --params '<json_params>'
```
...
```bash
lark-cli slides_ai xml_presentation.sildes delete --params '{
  "xml_presentation_id": "S7YwsFIGIlnS2qdscKDc1Yabcef",
  "slide_id": "sld_xxx"
}'
```
...
# 然后按已知 slide_id 删除
lark-cli slides_ai xml_presentation.sildes delete --params '{"xml_presentation_id":"S7YwsFIGIlnS2qdscKDc1Yabcef","slide_id":"sld_xxx"}'
```
...
1. **执行前必做**: 使用 `lark-cli schema slides_ai.xml_presentation.sildes.delete` 查看最新的参数结构
...
```bash
lark-cli slides_ai xml_presentation.sildes create --params '{"xml_presentation_id":"S7YwsFIGIlnS2qdscKDc1Yabcef"}' --data '{
  "slide": {
...
for slide_id in sld_a sld_b sld_c; do
  lark-cli slides_ai xml_presentation.sildes delete --params "{\"xml_presentation_id\":\"S7YwsFIGIlnS2qdscKDc1Yabcef\",\"slide_id\":\"$slide_id\"}"
done
```
