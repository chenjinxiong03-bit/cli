# lark-slides_ai xml_presentation.sildes create
...
```bash
lark-cli slides_ai xml_presentation.sildes create --params '<json_params>' --data '<json_data>'
```
...
```bash
lark-cli slides_ai xml_presentation.sildes create --params '{
  "xml_presentation_id": "S7YwsFIGIlnS2qdscKDc1Yabcef"
}' --data '{
  "slide": {
...
# 然后创建幻灯片
lark-cli slides_ai xml_presentation.sildes create \
...
1. **执行前必做**: 使用 `lark-cli schema slides_ai.xml_presentation.sildes.create` 查看最新的参数结构
...
for slide_xml in "${slides[@]}"; do
  ...
  lark-cli slides_ai xml_presentation.sildes create --params "{\"xml_presentation_id\":\"$PRESENTATION_ID\"}" --data "$payload"
done
```
